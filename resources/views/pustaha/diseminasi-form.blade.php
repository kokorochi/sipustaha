...
@php
    ...
    {      
        $diseminasi[$key] = old($key);
    }

    if(!isset($edit))
        $edit = false;
    if(!isset($diseminasi))
        $diseminasi = new \App\Diseminasi();
    ...
    <section id="page-content">
        ...
        <div id="tour-11" class="header-content">
            ...
            <div class="breadcrumb-wrapper hidden-xs">
                ...
                <ol class="breadcrumb">
                    <li class="active">Pustaha > Diseminasi</li>
                </ol>
            </div>
        </div><!-- /.header-content -->
        <!--/ End page header -->
        ...
        <div class="body-content animated fadeIn">
            <div class="panel rounded shadow">
                <div class="panel-heading">
                    <div class="pull-left">
                        <h3 class="panel-title">Detail Pustaha</h3>
                    </div>
                ...
                <div class="panel-body">
                    <div class="form-group">
                        ...
                        @foreach($approvales as $approval)
                            <div class="col-md-12">
                                @if($approval->code!="SB" && $approval->code!="UP" && $approval->code!="SBD")
                    ...
                    <div class="form-group is_simpel">
                     ...
                     {{$pustaha['is_simpel'] == '1' ? 'checked' : 'null'}} data-on-text="YA" data-off-text="TIDAK" data-on-color="teal" id="is_simpel" disabled>
                    </div>
                    <div id="research" {{$pustaha['is_simpel'] == '1' ? "null" : "style=display:none"}} class="form-group {{$errors->has('research_id') ? 'has-error' : null}}">
                        ...
                        @if($auth == 'SU')
                            <input name="research_full" type="text" class="form-control search-research" value="{{$pustaha['research_full']}}"
                               placeholder="Judul Penelitian" disabled>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="panel rounded shadow">
                        <div class="panel-heading">
                            <div class="pull-left">
                                <h3 class="panel-title">{{$page_title}} @if(!empty($disabled)) (*telah diajukan)@endif</h3>
                            </div>
                            <div class="pull-right">
                                <button class="btn btn-sm" data-action="collapse" data-container="body"
                                        data-toggle="tooltip"
                        ...
                        <div id="pustaha-container" class="panel-body">
                            @if($edit)
                                <a href="{{url('diseminasi/edit?id=' . $pustaha->id)}}" class="btn btn-success rounded">Ubah</a>
                            @endif
                            <form action="{{url($action_url)}}" method="post" enctype="multipart/form-data">
                                {{csrf_field()}}
                                <input name="pustaha_id" type="hidden" value="{{$_GET['id']}}">
                                <input name="upd_mode" type="hidden" value="{{$upd_mode}}">

                                @if($upd_mode == 'edit')
                                    <input type="hidden" name="id" value="{{$diseminasi['id']}}">
                                @endif
                                @include('layout.input-upload', ['passing_variable' => 'file_dissemination_ori', 'passing_description' => 'Lampiran Permohonan Bantuan Diseminasi (Cover, Biodata, Data isian, Surat Pernyataan Pengajuan) (*.pdf)', 'passing_type' => '1'])
                                @include('layout.input-upload', ['passing_variable' => 'file_iptek_ori', 'passing_description' => 'File (Bukti Penyebarluasan IPTEK) (*.pdf)', 'passing_type' => '2'])
                                @include('layout.input-upload', ['passing_variable' => 'file_presentation_ori', 'passing_description' => 'File Persentasi Seminar (*.pdf)', 'passing_type' => '3'])
                                @include('layout.input-upload', ['passing_variable' => 'file_poster_ori', 'passing_description' => 'File Poster (*.pdf)', 'passing_type' => '4'])
